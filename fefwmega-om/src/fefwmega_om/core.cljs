...
(defn card-component
  ...
  (reify
    ...
    (render [_]
      (sab/html
        (let [{:keys [app-state* n-lines line-idx column-idx card]} data*]
        (let [top-padding 64
          ...
          [:div.ffmg-deck-cell
           {:style
            {:max-height (adjust-height n-lines)
             :max-width  (adjust-height n-lines)
             :height     (calc-height n-lines "vw")
             :width      (calc-height n-lines "vw")}
            :on-click
